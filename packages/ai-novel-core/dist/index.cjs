...
__export(index_exports, {
  ...
  agentLabelFromType: () => agentLabelFromType,
  agentTypeFromLabel: () => agentTypeFromLabel,
  backfillPendingKnowledgeEmbeddings: () => backfillPendingKnowledgeEmbeddings,
  backfillPendingMemoryEmbeddings: () => backfillPendingMemoryEmbeddings,
  buildDirectorDiscussionMessage: () => buildDirectorDiscussionMessage,
  buildInitialSuperGraph: () => buildInitialSuperGraph,
  buildSuperGraphIndex: () => buildSuperGraphIndex,
  chunkKnowledgeContent: () => chunkKnowledgeContent,
  createAgentMessage: () => createAgentMessage,
  createAigcDetectorClient: () => createAigcDetectorClient,
  createArtifactMessage: () => createArtifactMessage,
  ...
  createLocalTextEmbedding: () => createLocalTextEmbedding,
  ...
  deriveProjectRuntimeState: () => deriveProjectRuntimeState,
  detectAigcSegments: () => detectAigcSegments,
  detectAigcText: () => detectAigcText,
  ensureEnvLlmConfigImported: () => ensureEnvLlmConfigImported,
  evaluateChapterConsistency: () => evaluateChapterConsistency,
  evaluateCharacterProfilePresence: () => evaluateCharacterProfilePresence,
  evaluateKnowledgeBenchmark: () => evaluateKnowledgeBenchmark,
  evaluateKnowledgeRetrieval: () => evaluateKnowledgeRetrieval,
  ...
  executeManualAdvanceCommand: () => executeManualAdvanceCommand,
  executeManualInterruptCommand: () => executeManualInterruptCommand,
  executeManualRetryChapterCommand: () => executeManualRetryChapterCommand,
  extractChinesePersonNames: () => extractChinesePersonNames,
  ...
  getCachedActiveLlmConfig: () => getCachedActiveLlmConfig,
  getFactoryDbPath: () => getFactoryDbPath,
  getNovelAutopilotWorkerStatus: () => getNovelAutopilotWorkerStatus,
  getProjectEnvCandidatePaths: () => getProjectEnvCandidatePaths,
  getProjectEnvPath: () => getProjectEnvPath,
  ...
  inferLockedProtagonistName: () => inferLockedProtagonistName,
  ingestGlobalWritingResources: () => ingestGlobalWritingResources,
  ingestKnowledgeSource: () => ingestKnowledgeSource,
  ingestProjectArtifact: () => ingestProjectArtifact,
  initAutonomousProject: () => initAutonomousProject,
  initializeSuperGraph: () => initializeSuperGraph,
  ...
  loadActiveLlmConfig: () => loadActiveLlmConfig,
  loadAndPruneGlobalContext: () => loadAndPruneGlobalContext,
  loadAutonomousState: () => loadAutonomousState,
  loadLlmConfigFromEnv: () => loadLlmConfigFromEnv,
  loadProductionWritingResources: () => loadProductionWritingResources,
  loadSuperGraph: () => loadSuperGraph,
  loadSuperGraphForUpdate: () => loadSuperGraphForUpdate,
  ...
  readProjectEnv: () => readProjectEnv,
  recordDirectorCommandEvent: () => recordDirectorCommandEvent,
  resolveFactoryRootDir: () => resolveFactoryRootDir,
  resolveManagedProjectRoot: () => resolveManagedProjectRoot,
  resolveProjectEnvWritePath: () => resolveProjectEnvWritePath,
  ...
  routeUserMessage: () => routeUserMessage,
  ...
  saveSuperGraph: () => saveSuperGraph,
  setCachedActiveLlmConfig: () => setCachedActiveLlmConfig,
  ...
  superGraphFromDbRows: () => superGraphFromDbRows,
  syncManagedProjectState: () => syncManagedProjectState,
  ...
  upsertDiscussionInSuperGraph: () => upsertDiscussionInSuperGraph,
  upsertProjectEnvValues: () => upsertProjectEnvValues,
...
function upsertProjectEnvValues(rootDir, updates) {
  ...
  const original = import_node_fs.default.existsSync(envPath) ? import_node_fs.default.readFileSync(envPath, "utf8") : "";
...
`;
  import_node_fs.default.writeFileSync(envPath, finalContent, "utf8");
}
...
var MANAGED_PROJECTS_SEGMENT2 = `${import_node_path4.default.sep}.ai-novel-projects${import_node_path4.default.sep}`;
...
function getAigcProjectEnvCandidatePaths(rootDir) {
  ...
  const candidates = [
    ...
    import_node_path4.default.join(resolvedRootDir, ...PACKAGE_ENV_PARTS2)
  ];
  ...
  if (workspaceRoot) {
    candidates.push(
      ...
      import_node_path4.default.join(workspaceRoot, ...PACKAGE_ENV_PARTS2)
    );
  }
...
function normalizeChapterRows(rows) {
  return toArray(rows).map((row) => ({
    ...
    status: normalizeTaskStatus2(row.status)
...
function workspacePath(rootDir, ...parts) {
  return import_node_path5.default.join(rootDir, WORKSPACE_DIR, ...parts);
}
function graphPaths(rootDir) {
  const graphDir = workspacePath(rootDir, "graph");
  return {
    graphDir,
    graphPath: import_node_path5.default.join(graphDir, "super-graph.json"),
    indexPath: import_node_path5.default.join(graphDir, "index.json"),
    violationsPath: import_node_path5.default.join(graphDir, "violations.json")
  };
}
...
async function upsertCheckpointInSuperGraph(rootDir, checkpoint, options = {}) {
  const graph = await loadSuperGraphForUpdate(rootDir, options);
  const checkpointId = `checkpoint:${import_node_path5.default.basename(checkpoint.path).replace(/\.json$/i, "")}`;
...
function normalizeRelativePath(value) {
  return value.split(import_node_path6.default.sep).join("/");
}
...
async function ingestKnowledgeSource(options) {
  return withFactoryDb(options.rootDir, async (db) => {
    ...
    const source = db.upsertKnowledgeSource({
      ...
      path: options.path,
      title: options.title || import_node_path6.default.basename(options.path),
      contentHash,
...
async function ingestGlobalWritingResources(rootDir, options = {}) {
  ...
  const candidates = [
    import_node_path6.default.join(rootDir, "packages", "ai-novel-core", "resources", "writing"),
  ...
  for (const filePath of files.slice(0, limit)) {
    ...
    const relativePath = normalizeRelativePath(import_node_path6.default.join("packages", "ai-novel-core", "resources", "writing", import_node_path6.default.relative(resourceRoot, filePath)));
    const result = await ingestKnowledgeSource({
      rootDir,
      scope: "global",
      sourceType: sourceTypeForPath(relativePath),
      path: relativePath,
      title: import_node_path6.default.basename(filePath),
      content,
      metadata: {
...
async function ingestProjectArtifact(options) {
  const absolutePath = import_node_path6.default.isAbsolute(options.artifactPath) ? options.artifactPath : import_node_path6.default.join(options.projectRoot, options.artifactPath);
  ...
  return ingestKnowledgeSource({
    ...
    title: import_node_path6.default.basename(options.artifactPath),
    content,
    metadata: {
      kind: options.kind,
      ...options.metadata || {}
...
function formatKnowledgeForPrompt(rows, limit = 8) {
  ...
  return rows.slice(0, limit).map((row, index) => {
    ...
    const title = String(source.title || row.source_title || import_node_path6.default.basename(pathValue) || "knowledge");
    return [
...
async function writeProjectRegistry(rootDir, projects) {
  ...
  await import_promises6.default.mkdir(projectsRoot, { recursive: true });
  await import_promises6.default.writeFile(registryPath, `${JSON.stringify(projects, null, 2)}
`);
}
...
async function writeWorkspaceArtifacts(rootDir, state) {
  ...
  await import_promises6.default.mkdir(paths.reportsDir, { recursive: true });
  await import_promises6.default.mkdir(paths.styleDir, { recursive: true });
  await import_promises6.default.mkdir(paths.chaptersDir, { recursive: true });
  await import_promises6.default.mkdir(paths.coverDir, { recursive: true });
  await import_promises6.default.mkdir(paths.comicDir, { recursive: true });
  await import_promises6.default.mkdir(paths.characterCoreDir, { recursive: true });
...
`
  ...
  await import_promises6.default.writeFile(import_node_path9.default.join(paths.promptsDir, "react-worldbuilding.md"), `${reactPrompt}
`);
  await import_promises6.default.writeFile(import_node_path9.default.join(paths.plansDir, "plan-and-solve-brief.md"), `${planBrief}
`);
  await import_promises6.default.writeFile(paths.consensusPath, `${globalConsensus}
`);
  await import_promises6.default.writeFile(paths.styleProfilePath, `${styleProfile}
`);
  await import_promises6.default.writeFile(paths.styleRulebookPath, `${styleRulebook}
`);
...
`);
  await import_promises6.default.writeFile(paths.relationsPath, `${relations}
`);
  await import_promises6.default.writeFile(paths.characterEvolutionPath, `${evolution}
`);
  await import_promises6.default.writeFile(import_node_path9.default.join(paths.coverDir, "cover-brief.md"), `${coverBrief}
`);
  await import_promises6.default.writeFile(import_node_path9.default.join(paths.comicDir, "comic-plan.md"), `${comicPlan}
`);
  await Promise.all(
    AGENT_ROLES.flatMap((role) => {
      const base = import_node_path9.default.join(paths.agentPromptsDir, `${role}.base.md`);
      const dynamic = import_node_path9.default.join(paths.agentPromptsDir, `${role}.dynamic.md`);
      return [
        import_promises6.default.writeFile(base, `${basePrompts.get(role) ?? ""}
`),
        import_promises6.default.writeFile(dynamic, `${dynamicPrompts.get(role) ?? ""}
`)
      ];
    })
  );
...
async function initAutonomousProject(options) {
  ...
  await import_promises6.default.mkdir(paths.workspaceDir, { recursive: true });
...
async function deleteManagedAutonomousProject(rootDir, projectId) {
  ...
  const expectedProjectRoot = import_node_path9.default.resolve(resolveProjectRoot(rootDir, project.id));
  if (import_node_path9.default.resolve(projectRoot) !== expectedProjectRoot) {
  ...
  await import_promises6.default.rm(projectRoot, { recursive: true, force: true });
...
async function prepareCoverGeneration(rootDir) {
  ...
  await import_promises6.default.writeFile(paths.coverPromptPath, `${prompt}
`);
...
async function reviewInterruption(options) {
  ...
  const logPath = import_node_path9.default.join(reportsDir, "interruptions.log.md");
...
function getWorkspaceSummary(rootDir) {
  ...
  return {
    ...
    slug: slugifyTitle(import_node_path9.default.basename(rootDir))
  };
}
