...
__export(writing_pipeline_exports, {
  ProductionPlanningBlockedError: () => ProductionPlanningBlockedError,
  ...
  commitChapterProductionStage: () => commitChapterProductionStage,
  ...
  evaluateSceneCardCharacterObligations: () => evaluateSceneCardCharacterObligations,
  evaluateSemanticPreservation: () => evaluateSemanticPreservation,
  evaluateUnplannedCharacterDrift: () => evaluateUnplannedCharacterDrift,
  ...
  prepareChapterProductionInputs: () => prepareChapterProductionInputs,
  ...
  runChapterNaturalnessStage: () => runChapterNaturalnessStage,
  runChapterProductionPipeline: () => runChapterProductionPipeline,
  runQualityGateWithRevisions: () => runQualityGateWithRevisions,
  sanitizeKnownCastNames: () => sanitizeKnownCastNames,
...
var requireBuiltin = (0, import_node_module.createRequire)(import_node_path4.default.join(process.cwd(), "ai-novel-factory-runtime.js"));
...
function readAigcSettingsEnv(rootDir) {
  ...
  try {
    const dbPath = import_node_path4.default.join(factoryRoot, ".ai-novel-factory", "factory.sqlite");
...
async function blockMasterPlanningWithoutSourceProtagonist(projectRoot, paths, state, context, options) {
  ...
  await import_promises4.default.mkdir(paths.plansDir, { recursive: true });
  const briefPath = import_node_path6.default.join(paths.plansDir, "master-planning-protagonist-brief.md");
  ...
  await import_promises4.default.writeFile(briefPath, `${brief}
`);
...
async function blockStoryFoundationWithoutMasterProtagonist(projectRoot, paths, masterOutline, options) {
  ...
  const namingBriefPath = import_node_path6.default.join(paths.plansDir, "protagonist-naming-brief.md");
  ...
  await import_promises4.default.writeFile(namingBriefPath, `${namingBrief}
`);
...
`);
  ...
  await emitWritingProgress(options, {
    ...
    artifacts: written.map((artifactPath) => ({
      ...
      label: import_node_path6.default.basename(artifactPath),
      kind: "plan",
      status: "completed"
    })),
    ...
    workflow: {
      ...
      expandableArtifactPath: relativeArtifactPath(projectRoot, import_node_path6.default.join(paths.plansDir, "story-bible.md"))
    }
  });
  return written;
}
