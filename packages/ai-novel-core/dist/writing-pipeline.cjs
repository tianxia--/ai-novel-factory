...
var FactoryDb = class _FactoryDb {
  ...
  getChapterFacts(projectId, now = /* @__PURE__ */ new Date()) {
    ...
    for (const fact of [...facts.values()].sort((left, right) => left.chapterNumber - right.chapterNumber)) {
      if (fact.finalPath) {
        const finalText = readTextFileSync(import_node_path.default.isAbsolute(fact.finalPath) ? fact.finalPath : import_node_path.default.join(projectRoot, fact.finalPath));
        if (finalText) {
...
var MANAGED_PROJECTS_SEGMENT = `${import_node_path2.default.sep}.ai-novel-projects${import_node_path2.default.sep}`;
function uniquePaths(paths) {
  return [...new Set(paths.map((candidate) => import_node_path2.default.resolve(candidate)))];
}
function inferWorkspaceRootFromManagedProject(rootDir) {
  ...
  return rootDir.slice(0, index) || import_node_path2.default.parse(rootDir).root;
}
...
function getProjectEnvPath(rootDir = process.cwd()) {
  return import_node_path2.default.resolve(rootDir, ".env");
}
function getProjectEnvCandidatePaths(rootDir = process.cwd()) {
  const resolvedRootDir = import_node_path2.default.resolve(rootDir);
  const candidates = [
    getProjectEnvPath(resolvedRootDir),
    import_node_path2.default.join(resolvedRootDir, ...PACKAGE_ENV_PARTS)
  ];
  const workspaceRoot = inferWorkspaceRootFromManagedProject(resolvedRootDir);
  if (workspaceRoot) {
    candidates.push(
      getProjectEnvPath(workspaceRoot),
      import_node_path2.default.join(workspaceRoot, ...PACKAGE_ENV_PARTS)
    );
  }
...
function readProjectEnv(rootDir = process.cwd()) {
  for (const envPath of getProjectEnvCandidatePaths(rootDir)) {
    if (import_node_fs2.default.existsSync(envPath)) {
      const raw = import_node_fs2.default.readFileSync(envPath, "utf8");
      return {
        envPath,
        exists: true,
...
function workspaceRootForProject(projectRoot) {
  const marker = `${import_node_path4.default.sep}.ai-novel-projects${import_node_path4.default.sep}`;
...
async function readOptionalText(filePath) {
  try {
    return (await import_promises3.default.readFile(filePath, "utf8")).trim();
  } catch {
    return "";
  }
}
function relativeArtifactPath(projectRoot, absolutePath) {
  return import_node_path4.default.relative(projectRoot, absolutePath).replaceAll(import_node_path4.default.sep, "/");
}
...
async function loadProductionWritingResources(rootDir) {
  ...
  const candidates = [
    ...
    import_node_path4.default.join(workspaceRoot, "packages", "ai-novel-core", "resources", "writing"),
  ...
  const loadPromise = (async () => {
    const find = async (relativePath) => {
      for (const candidate of candidates) {
        const text = await readOptionalText(import_node_path4.default.join(candidate, relativePath));
        if (text) return text;
      }
      return "";
    };
...
async function writeProductionWritingResourceArtifacts(projectRoot, paths, state, options = {}) {
  ...
  const resourceDir = import_node_path4.default.join(paths.styleDir, "production-resources");
  await import_promises3.default.mkdir(resourceDir, { recursive: true });
  const guidePath = import_node_path4.default.join(resourceDir, "production-writing-assets.md");
  const genre = inferGenreProfile(state);
  const content = [
  ...
  await import_promises3.default.writeFile(guidePath, content);
  if (options.factoryRootDir && options.projectId) {
    await withFactoryDb(options.factoryRootDir, async (db) => {
      db.recordArtifact({
        projectId: options.projectId,
...
async function writeProductionMasterOutline(projectRoot, paths, state, context, options = {}) {
  ...
  await import_promises3.default.mkdir(paths.plansDir, { recursive: true });
  await import_promises3.default.writeFile(paths.masterOutlinePath, `${content}
`);
...
async function writeAllDetailedChapterBlueprints(projectRoot, paths, state, context, options = {}) {
  const resources = await loadProductionWritingResources(projectRoot);
  await import_promises3.default.mkdir(paths.chapterBlueprintsDir, { recursive: true });
  const written = [];
  for (const task of state.plan.chapterTasks) {
    ...
    const blueprintPath = import_node_path4.default.join(paths.chapterBlueprintsDir, `chapter-${String(task.chapterNumber).padStart(3, "0")}.md`);
    await import_promises3.default.writeFile(blueprintPath, `${content}
`);
...
async function runChapterProductionPipeline(projectRoot, paths, state, task, options = {}) {
  ...
  const previousMemory = previousChapterId ? await readOptionalText(import_node_path4.default.join(paths.memoryDir, `${previousChapterId}-memory.md`)) : "";
  const previousFinalDraft = previousChapterId ? await readOptionalText(import_node_path4.default.join(paths.chaptersDir, `${previousChapterId}.final.md`)) : "";
  const blueprintPath = import_node_path4.default.join(paths.chapterBlueprintsDir, `${chapterId}.md`);
  const context = {
  ...
  if (!hasCausalBlueprint(blueprint)) {
    ...
    await import_promises3.default.mkdir(paths.chapterBlueprintsDir, { recursive: true });
    await import_promises3.default.writeFile(blueprintPath, `${blueprint}
`);
    await recordPipelineArtifact(projectRoot, blueprintPath, "plan", options, {
      chapterNumber: task.chapterNumber,
      stage: "drafting",
  ...
  throwIfPipelineAborted(options);
  const draftPath = import_node_path4.default.join(paths.chaptersDir, `${chapterId}.draft.md`);
  const reviewedPath = import_node_path4.default.join(paths.chaptersDir, `${chapterId}.reviewed.md`);
  const finalPath = import_node_path4.default.join(paths.chaptersDir, `${chapterId}.final.md`);
  const reportPath = import_node_path4.default.join(paths.reportsDir, `${chapterId}-quality.md`);
  const memoryPath = import_node_path4.default.join(paths.memoryDir, `${chapterId}-memory.md`);
  await import_promises3.default.mkdir(paths.chaptersDir, { recursive: true });
  await import_promises3.default.mkdir(paths.reportsDir, { recursive: true });
  await import_promises3.default.mkdir(paths.memoryDir, { recursive: true });
  await import_promises3.default.writeFile(draftPath, `${draft}
`);
  await import_promises3.default.writeFile(reportPath, `${report}
`);
  await import_promises3.default.writeFile(reviewedPath, `${draft}

---

${report}
`);
  await import_promises3.default.writeFile(finalPath, `${finalDraft}
`);
  await import_promises3.default.writeFile(memoryPath, `${memoryUpdate}
`);
