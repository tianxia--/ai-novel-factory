...
async function writeJsonFileAtomic(filePath, value) {
  await import_promises3.default.mkdir(import_node_path5.default.dirname(filePath), { recursive: true });
  const tempPath = import_node_path5.default.join(import_node_path5.default.dirname(filePath), `.${import_node_path5.default.basename(filePath)}.${Date.now()}.tmp`);
...
async function loadAndPruneGlobalContext(params) {
  ...
  if (paths && previousChapterId) {
    const previousFinalDraft = await readOptionalText(import_node_path5.default.join(paths.chaptersDir, `${previousChapterId}.final.md`));
  ...
  let rawMemory = "";
  if (paths && previousChapterId) {
    rawMemory = await readOptionalText(import_node_path5.default.join(paths.memoryDir, `${previousChapterId}-memory.md`));
  }
