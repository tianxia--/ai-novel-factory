...
function readProjectEnv(rootDir = process.cwd()) {
  ...
  const existingPaths = candidates.filter((envPath) => import_node_fs2.default.existsSync(envPath));
...
var MANAGED_PROJECTS_SEGMENT2 = `${import_node_path5.default.sep}.ai-novel-projects${import_node_path5.default.sep}`;
...
function getAigcProjectEnvCandidatePaths(rootDir) {
  ...
  const candidates = [
    import_node_path5.default.join(resolvedRootDir, ".env"),
    import_node_path5.default.join(resolvedRootDir, ...PACKAGE_ENV_PARTS2)
  ];
  const workspaceRoot = inferAigcWorkspaceRootFromManagedProject(resolvedRootDir);
  if (workspaceRoot) {
    candidates.push(
      import_node_path5.default.join(workspaceRoot, ".env"),
      import_node_path5.default.join(workspaceRoot, ...PACKAGE_ENV_PARTS2)
    );
  }
  return [...new Set(candidates.map((candidate) => import_node_path5.default.resolve(candidate)))];
}
function inferAigcWorkspaceRootFromManagedProject(rootDir) {
  ...
  return rootDir.slice(0, index) || import_node_path5.default.parse(rootDir).root;
}
...
async function readCharacterDossiers(filePath) {
  if (!filePath) return [];
  try {
    const parsed = JSON.parse(await import_promises3.default.readFile(filePath, "utf8"));
...
async function writeJsonFileAtomic(filePath, value) {
  await import_promises3.default.mkdir(import_node_path6.default.dirname(filePath), { recursive: true });
  ...
  await import_promises3.default.writeFile(tempPath, `${JSON.stringify(value, null, 2)}
`);
...
async function updateStyleFingerprintFromFirstChapter(input) {
  ...
  await import_promises3.default.writeFile(input.paths.styleProfilePath, `${updatedStyleProfile.trimEnd()}
`);
  return extracted;
}
...
`);
  await import_promises3.default.writeFile(reportPath, `${reportWithAigcDetection}
`);
...
`);
  ...
  if (paths.characterDossiersMarkdownPath && updatedCharacterDossiers.length) {
    await import_promises3.default.writeFile(paths.characterDossiersMarkdownPath, `${formatCharacterDossiersMarkdown(updatedCharacterDossiers)}
`);
  }
