    ...
    if (projectRoot && stateObj) {
      ...
      for (const [pathKey, row] of artifactsByPath.entries()) {
        if (pathKey.includes("/consensus/discussion-") || pathKey.includes(".ai-novel/consensus/discussion-")) {
          try {
            const absPath = import_node_fs2.default.existsSync(pathKey) ? pathKey : import_node_path2.default.resolve(projectRoot, pathKey);
            if (import_node_fs2.default.existsSync(absPath)) {
              const mtime = import_node_fs2.default.statSync(absPath).mtimeMs;
      ...
      for (const [pathKey, row] of artifactsByPath.entries()) {
        ...
        if (isOutline || isBlueprint || isDraft || isLatestDiscussion) {
          try {
            const absPath = import_node_fs2.default.existsSync(pathKey) ? pathKey : import_node_path2.default.resolve(projectRoot, pathKey);
            if (import_node_fs2.default.existsSync(absPath)) {
              row.content = import_node_fs2.default.readFileSync(absPath, "utf8");
            }
          } catch (e) {
...
async function writeAutopilotCheckpoint(projectRoot, state, label, details = {}) {
  const checkpointDir = import_node_path12.default.join(projectRoot, ".ai-novel", "checkpoints");
  await import_promises9.default.mkdir(checkpointDir, { recursive: true });
  ...
  await import_promises9.default.writeFile(
    import_node_path12.default.join(checkpointDir, fileName),
...
function parseFlags(argv) {
  ...
  return {
    rootDir: import_node_path13.default.resolve(flags.get("root-dir") || process.cwd()),
