...
var workflow_debug_branch_exports = {};
__export(workflow_debug_branch_exports, {
  FactoryWorkflowDebugBranchManager: () => FactoryWorkflowDebugBranchManager,
  diffWorkflowState: () => diffWorkflowState
});
module.exports = __toCommonJS(workflow_debug_branch_exports);
...
function jsonPointer(path2, key) {
  const escaped = String(key).replace(/~/gu, "~0").replace(/\//gu, "~1");
  return `${path2}/${escaped}`;
}
function diffWorkflowState(before, after, path2 = "") {
  if (Object.is(before, after)) return [];
  if (Array.isArray(before) && Array.isArray(after)) {
    const changes = [];
    const length = Math.max(before.length, after.length);
    for (let index = 0; index < length; index += 1) {
      const nextPath = jsonPointer(path2, index);
  ...
  if (isRecord(before) && isRecord(after)) {
    ...
    for (const key of Array.from(keys).sort()) {
      const nextPath = jsonPointer(path2, key);
  ...
  return [{ operation: "replace", path: path2 || "/", before, after }];
}
...
function nextCheckpointId() {
  return (0, import_langgraph_checkpoint2.uuid6)(Math.floor(Math.random() * 16384));
}
