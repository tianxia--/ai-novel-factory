...
function currentBundleDir() {
  ...
  for (const line of stack.split("\n")) {
    ...
    if (fileUrlMatch) {
      return import_node_path2.default.dirname(decodeURIComponent(fileUrlMatch[1]));
    }
    const fileMatch = line.match(/\((\/[^():]+):\d+:\d+\)/) || line.match(/at (\/[^():]+):\d+:\d+/);
    if (fileMatch) {
      return import_node_path2.default.dirname(fileMatch[1]);
    }
  }
  return process.cwd();
}
...
async function ingestKnowledgeSource(options) {
  return withFactoryDb(options.rootDir, async (db) => {
    ...
    const source = db.upsertKnowledgeSource({
      ...
      path: options.path,
      title: options.title || import_node_path2.default.basename(options.path),
      contentHash,
...
async function ingestGlobalWritingResources(rootDir, options = {}) {
  ...
  const candidates = [
    import_node_path2.default.join(rootDir, "packages", "ai-novel-core", "resources", "writing"),
    import_node_path2.default.resolve(bundleDir, "..", "resources", "writing"),
    import_node_path2.default.join(process.cwd(), "packages", "ai-novel-core", "resources", "writing")
  ];
  let resourceRoot = "";
  ...
  for (const filePath of files.slice(0, limit)) {
    ...
    const relativePath = normalizeRelativePath(import_node_path2.default.join("packages", "ai-novel-core", "resources", "writing", import_node_path2.default.relative(resourceRoot, filePath)));
    const result = await ingestKnowledgeSource({
      rootDir,
      scope: "global",
      sourceType: sourceTypeForPath(relativePath),
      path: relativePath,
      title: import_node_path2.default.basename(filePath),
      content,
      metadata: {
...
async function ingestProjectArtifact(options) {
  const absolutePath = import_node_path2.default.isAbsolute(options.artifactPath) ? options.artifactPath : import_node_path2.default.join(options.projectRoot, options.artifactPath);
  ...
  return ingestKnowledgeSource({
    ...
    title: import_node_path2.default.basename(options.artifactPath),
    content,
    metadata: {
      kind: options.kind,
      ...options.metadata || {}
...
function formatKnowledgeForPrompt(rows, limit = 8) {
  ...
  return rows.slice(0, limit).map((row, index) => {
    ...
    const title = String(source.title || row.source_title || import_node_path2.default.basename(pathValue) || "knowledge");
    return [
