...
__export(studio_server_exports, {
  handleNovelStudioApi: () => handleNovelStudioApi,
  runNovelStudioServerCli: () => runNovelStudioServerCli,
  startNovelStudioServer: () => startNovelStudioServer,
  writeServerErrorResponse: () => writeServerErrorResponse
});
...
async function readDiscussionTranscript(rootDir) {
  try {
    return await import_promises9.default.readFile(import_node_path12.default.join(rootDir, ".ai-novel", "chat", "discussion-log.md"), "utf8");
  } catch {
    return "";
  }
}
async function appendAutopilotSubmissionTranscript(rootDir, input) {
  ...
  const chatDir = import_node_path12.default.join(rootDir, ".ai-novel", "chat");
  const submittedAt = (/* @__PURE__ */ new Date()).toISOString();
  await import_promises9.default.mkdir(chatDir, { recursive: true });
  await import_promises9.default.appendFile(
    import_node_path12.default.join(chatDir, "discussion-log.md"),
    [
      `## ${submittedAt}`,
      `User: ${message}`,
...
function messagePart3(messageId, index, type, data, createdAt) {
  return {
    id: `${messageId}:part:${index}`,
    messageId,
    index,
...
function userMessageParts(messageId, input) {
  return [
    messagePart3(messageId, 0, "text", { text: input.content }, input.createdAt),
    messagePart3(messageId, 1, "json", {
...
function statusMessageParts(messageId, input) {
  ...
  return [
    messagePart3(messageId, 0, "markdown", { text }, input.createdAt),
    messagePart3(messageId, 1, "json", {
...
function toolMessageParts(messageId, input) {
  const parts = [
    messagePart3(messageId, 0, "markdown", { text: input.content || `${input.toolName}: ${input.status}` }, input.createdAt),
    messagePart3(messageId, 1, "tool_call", {
      toolName: input.toolName,
      input: input.toolInput ?? null
    }, input.createdAt)
  ];
  parts.push(messagePart3(messageId, 2, input.error ? "tool_result" : "tool_result", {
    status: input.status,
...
async function writeKnowledgeEvaluationArtifact(rootDir, projectRoot, projectId, evaluation) {
  ...
  const absolutePath = import_node_path12.default.join(projectRoot, relativePath);
  await import_promises9.default.mkdir(import_node_path12.default.dirname(absolutePath), { recursive: true });
...
function messageRowsToEntries(rows = [], { limit = 80, transcriptBytes = 0 } = {}) {
  ...
  const artifactPathFromParts = (parts) => {
    ...
    const path13 = artifactData?.path || artifactData?.artifactPath || artifactData?.url || "";
    return typeof path13 === "string" ? path13 : "";
  };
...
async function readWorkspaceText(rootDir, ...parts) {
  try {
    return await import_promises9.default.readFile(import_node_path12.default.join(rootDir, ".ai-novel", ...parts), "utf8");
  } catch {
    return "";
  }
}
...
async function readWorkspaceArtifactText(rootDir, artifactPath) {
  ...
  try {
    return await import_promises9.default.readFile(import_node_path12.default.join(rootDir, normalized), "utf8");
  } catch {
    return "";
  }
}
...
function stateWithChapterFactsForPayload(state, factorySnapshot) {
  ...
  const normalizeTaskStatus3 = (value) => value === "pending" || value === "in_progress" || value === "complete" || value === "blocked" ? value : null;
  const normalizeGateStatus = (value) => value === "passed" || value === "needs_revision" || value === "blocked" ? value : null;
  const timestampMs3 = (value) => {
    if (typeof value !== "string" || value.trim().length === 0) {
      return 0;
    }
    const parsed = Date.parse(value);
  ...
  nextState.plan.chapterTasks = state.plan.chapterTasks.map((task) => {
    ...
    const factStatus = normalizeTaskStatus3(fact.status);
    const gateStatus = normalizeGateStatus(qualityGate?.status);
    const latestTaskStatusAt = timestampMs3(fact.latestTaskStatusAt);
    const qualityGateUpdatedAt = timestampMs3(
...
function deriveProductionObservability(factorySnapshot, resolvedState, contextPacket) {
  ...
  const staleJobsList = activeJobs.filter((job) => {
    ...
    return new Date(String(job.lease_expires_at)) < now2;
  }).map((job) => ({
    id: String(job.id || ""),
...
async function createWorkspacePayload(projectRoot, state, options = {}) {
  ...
  const graphDir = import_node_path12.default.join(projectRoot, ".ai-novel", "graph");
  const fileGraphIndex = await import_promises9.default.readFile(import_node_path12.default.join(graphDir, "index.json"), "utf8").then((raw) => JSON.parse(raw)).catch(() => null);
  const fileGraphViolations = await import_promises9.default.readFile(import_node_path12.default.join(graphDir, "violations.json"), "utf8").then((raw) => JSON.parse(raw)).catch(() => []);
  ...
  if (method === "POST" && requestPathname === "/api/knowledge/evaluate") {
    ...
    const cases = explicitCases.length > 0 ? explicitCases.map((item, index) => {
      ...
      return (Array.isArray(snapshot.knowledge?.citations) ? snapshot.knowledge.citations : []).map((citation, index) => {
        const expectedChunkIds = readJsonArray2(citation.used_chunk_ids_json).map((id) => String(id)).filter(Boolean);
        return {
...
function resolveStaticFile(staticDir, pathname) {
  ...
  return import_node_path12.default.join(staticDir, relative);
}
