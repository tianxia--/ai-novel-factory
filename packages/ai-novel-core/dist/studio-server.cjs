...
function stateWithChapterFactsForPayload(state, factorySnapshot) {
  ...
  const normalizeTaskStatus2 = (value) => value === "pending" || value === "in_progress" || value === "complete" || value === "blocked" ? value : null;
  ...
  nextState.plan.chapterTasks = state.plan.chapterTasks.map((task) => {
    ...
    const factStatus = normalizeTaskStatus2(fact.status);
