...
var GENERIC_IDEA_TERMS = /* @__PURE__ */ new Set([
  "\u6545\u4E8B",
  "\u5C0F\u8BF4",
  "\u4E3B\u89D2",
  "\u4E3B\u4EBA\u516C",
...
var ENGLISH_IDEA_STOPWORDS = /* @__PURE__ */ new Set([
  "about",
  "after",
  "against",
  "and",
...
function extractEnglishStorySignals(projectIdea) {
  const source = projectIdea.toLowerCase();
  const signals = [];
  const add = (term, pattern) => {
...
function fallbackProjectIdeaTerms(projectIdea) {
...
function extractProjectIdeaTerms(projectIdea) {
  const quotedTerms = Array.from(projectIdea.matchAll(/[「《“"]([^」》”"]{2,24})[」》”"]/gu)).map((match) => match[1]?.trim()).filter(Boolean);
  const englishStorySignals = extractEnglishStorySignals(projectIdea);
  const normalized = projectIdea.replace(/[「」《》“”"']/gu, " ").replace(/一名|一个|一种|发现|追查|调查|寻找|守住|穿越到|穿越|进入|见证|改变|成为|并|用|从|被|把|将|在|到|的|了|和|与|及|以及/gu, " ").replace(/[，。；：、,.!?/\\()[\]{}<>|_-]+/gu, " ");
  const chineseTerms = Array.from(normalized.matchAll(/[\u4e00-\u9fff]{2,10}/gu)).map((match) => match[0]);
  const englishTerms = Array.from(normalized.matchAll(/[A-Za-z][A-Za-z0-9'-]{3,}/gu)).map((match) => match[0]).filter((term) => {
  ...
  ].map((term) => term.trim()).filter((term) => term && !GENERIC_IDEA_TERMS.has(term));
...
function summarizeProjectIdeaForCausalPlan(projectIdea) {
...
`);
  ...
  if (state.runtime.stage === "setting_review") {
    throwIfStopped(pipelineOptions.signal);
    const settingReviewApprovalPath = path4.join(paths.plansDir, SETTING_REVIEW_APPROVAL_FILE);
...
async function retryChapterProduction(rootDir, chapterNumber, options = {}) {
  ...
  if (pipelineOptions.factoryRootDir && pipelineOptions.projectId) {
    await withFactoryDb(pipelineOptions.factoryRootDir, async (db) => {
      db.markStreamingMessagesFailed(pipelineOptions.projectId, {
