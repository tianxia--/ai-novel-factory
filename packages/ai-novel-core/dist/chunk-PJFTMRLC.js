  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        ...
        modelName,
        apiMode,
        message: fallbackErrorMessage ? `Generation fallback failed: ${fallbackErrorMessage} (/models returned ${response.status} ${response.statusText || "unsupported"}).` : `Provider test failed with status ${response.status}.`
      };
    }
    const payload = await response.json();
...
function stripStyleSampleLengthInstructions(value = "") {
  return value.replace(/(?:字数|长度|篇幅|控制在|不少于|不低于|至少|最多|约)\s*[0-9０-９]{2,5}\s*(?:[-—~至到]\s*[0-9０-９]{2,5})?\s*(?:字|个中文字符|字符|tokens?|词)?/giu, "\u6837\u6BB5\u957F\u5EA6\u670D\u4ECE\u672C\u8F6E\u5F53\u524D\u7EA6\u675F").replace(/[0-9０-９]{2,5}\s*(?:[-—~至到]\s*[0-9０-９]{2,5})\s*(?:字|个中文字符|字符|tokens?|词)/giu, "\u672C\u8F6E\u6307\u5B9A\u957F\u5EA6");
}
...
async function blockMasterPlanningWithoutSourceProtagonist(projectRoot, paths, state, context, options) {
  ...
  await fs2.mkdir(paths.plansDir, { recursive: true });
  const briefPath = path3.join(paths.plansDir, "master-planning-protagonist-brief.md");
  ...
  await fs2.writeFile(briefPath, `${brief}
`);
...
async function blockStoryFoundationWithoutMasterProtagonist(projectRoot, paths, masterOutline, options) {
  ...
  const namingBriefPath = path3.join(paths.plansDir, "protagonist-naming-brief.md");
  ...
  await fs2.writeFile(namingBriefPath, `${namingBrief}
`);
...
`);
  ...
  await emitWritingProgress(options, {
    ...
    artifacts: written.map((artifactPath) => ({
      ...
      label: path3.basename(artifactPath),
      kind: "plan",
      status: "completed"
    })),
    ...
    workflow: {
      ...
      expandableArtifactPath: relativeArtifactPath(projectRoot, path3.join(paths.plansDir, "story-bible.md"))
    }
  });
  return written;
}
