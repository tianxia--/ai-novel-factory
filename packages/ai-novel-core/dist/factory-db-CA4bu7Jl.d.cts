import { A as AutonomousNovelState, a as NovelProjectRecord, N as NovelStage } from './cli-types-3z9cP1VA.cjs';
...
interface WorkflowStepAttemptInput {
    id: string;
    stepId: string;
    runId: string;
    projectId: string;
...
interface WorkflowCheckpointQuery {
    threadId: string;
...
declare class FactoryDb {
    ...
    createWorkflowStep(input: WorkflowStepInput): void;
    updateWorkflowStep(stepId: string, status: StepStatus, patch?: {
        output?: unknown;
        error?: string | null;
        validationStatus?: WorkflowStepInput["validationStatus"];
        metadata?: unknown;
    }): void;
    createWorkflowStepAttempt(input: WorkflowStepAttemptInput): void;
    updateWorkflowStepAttempt(attemptId: string, status: StepStatus, patch?: {
        output?: unknown;
        error?: unknown;
        usage?: unknown;
        metadata?: unknown;
    }): void;
    listWorkflowSteps(runId: string): DbRecord[];
    getWorkflowRun(runId: string): DbRecord | null;
    listWorkflowStepAttempts(stepId: string): DbRecord[];
    ...
    updateWorkflowCheckpoint(checkpointRecordId: string, patch: {
        drift?: unknown;
        state?: unknown;
    }): void;
    deleteWorkflowCheckpointThread(projectId: string, threadId: string): void;
...
export { type AgentTurnInput as A, saveSuperGraph as B, type ChapterProductionFact as C, type DiscussionTarget as D, superGraphFromDbRows as E, type FactoryOperationalStatus as F, targetToArtifactKind as G, upsertCheckpointInSuperGraph as H, upsertDiscussionInSuperGraph as I, validateSuperGraph as J, type KnowledgeScope as K, withFactoryDb as L, type MessageListOptions as M, type ProjectSnapshot as P, type StepStatus as S, type WorkflowCheckpointQuery as W, type KnowledgeRecallRow as a, type ArtifactRecordInput as b, type CheckpointInput as c, FactoryDb as d, type KnowledgeChunkInput as e, type KnowledgeChunkRow as f, type KnowledgeCitationInput as g, type KnowledgeSourceInput as h, type SuperGraph as i, type SuperGraphEdge as j, type SuperGraphEdgeType as k, type SuperGraphNode as l, type SuperGraphNodeType as m, type SuperGraphValidationIssue as n, type WorkflowRunInput as o, type WorkflowStepAttemptInput as p, type WorkflowStepInput as q, buildInitialSuperGraph as r, buildSuperGraphIndex as s, getFactoryDbPath as t, initializeSuperGraph as u, loadSuperGraph as v, loadSuperGraphForUpdate as w, makeAgentTurnId as x, makeRunId as y, runMultiAgentDiscussion as z };
