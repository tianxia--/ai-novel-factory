...
import { withFactoryDb, type FactoryDb, type KnowledgeRecallRow, type KnowledgeScope } from "./factory-db"

const LOCAL_KNOWLEDGE_EMBEDDING_MODEL = "local-hash-v1"
const DEFAULT_CHUNK_CHAR_LIMIT = 1400
const DEFAULT_GLOBAL_RESOURCE_LIMIT = process.env.AI_NOVEL_TEST_MODE === "1" ? 12 : 5000
const DEFAULT_ARTIFACT_CHUNK_LIMIT = process.env.AI_NOVEL_TEST_MODE === "1" ? 40 : 500
...
export interface IngestKnowledgeSourceOptions {
  rootDir: string
...
export interface KnowledgeBenchmarkCaseResult extends KnowledgeRetrievalEvaluation {
  name: string
  query: string
}
...
function chapterNumberFromKnowledgePath(value: unknown) {
...
function sourceTypeForPath(filePath: string) {
...
function chunkTypeForHeading(heading: string, sourceType: string) {
...
function inferSceneTypes(text: string) {
  const scenes: string[] = []
...
function splitLongText(text: string, maxChars = DEFAULT_CHUNK_CHAR_LIMIT) {
...
function chunkMarkdown(content: string, sourceType: string, maxChars: number): KnowledgeChunkDraft[] {
  const lines = content.split("\n")
  const sections: Array<{ heading: string; body: string[] }> = []
  let current = { heading: "Document", body: [] as string[] }
...
function chunkVocabularyJson(content: string, maxEntries = DEFAULT_GLOBAL_RESOURCE_LIMIT): KnowledgeChunkDraft[] {
  try {
    const parsed = JSON.parse(content) as {
      metadata?: Record<string, unknown>
      categories?: Record<string, { title?: string; count?: number }>
    ...
    const wordChunks = Object.entries(parsed.word_index || {})
      .slice(0, maxEntries)
      .map(([word, value]) => {
...
export function chunkKnowledgeContent(input: {
  ...
  const chunks = normalizedPath.endsWith(".json")
    ? chunkVocabularyJson(input.content, limit)
    : chunkMarkdown(input.content, input.sourceType, maxChars)
  return chunks
    .filter((chunk) => chunk.content.trim().length > 0)
    .slice(0, limit)
}

export async function ingestKnowledgeSource(options: IngestKnowledgeSourceOptions) {
...
export async function ingestGlobalWritingResources(rootDir: string, options: { limit?: number } = {}) {
  ...
  for (const filePath of files.slice(0, limit)) {
    ...
    const result = await ingestKnowledgeSource({
      ...
      chunkLimit: filePath.endsWith("vocabulary_index.json") ? DEFAULT_GLOBAL_RESOURCE_LIMIT : undefined,
    })
    sources += 1
...
export async function ingestProjectArtifact(options: {
  rootDir: string
  projectId: string
  projectRoot: string
  artifactPath: string
  ...
  const absolutePath = path.isAbsolute(options.artifactPath)
    ? options.artifactPath
    : path.join(options.projectRoot, options.artifactPath)
...
export async function retrieveKnowledge(options: KnowledgeRetrieveOptions): Promise<KnowledgeRecallRow[]> {
  const results = await withFactoryDb(options.rootDir, async (db) => {
    ...
    const resetFacts = options.projectId
      ? db.getChapterFacts(options.projectId)
        .filter((fact) => fact.resetAt)
        .map((fact) => ({ chapterNumber: fact.chapterNumber, resetAt: fact.resetAt || "" }))
      : []
...
export function formatKnowledgeForPrompt(rows: KnowledgeRecallRow[], limit = 8) {
  if (!rows.length) {
...
export async function evaluateKnowledgeBenchmark(rootDir: string, cases: KnowledgeBenchmarkCase[]): Promise<KnowledgeBenchmarkResult> {
  const caseResults: KnowledgeBenchmarkCaseResult[] = []
  ...
  const sum = (field: "hitAtK" | "recallAtK" | "precisionAtK") => caseResults.reduce((total, item) => total + item[field], 0)
  return {
    cases: caseResults,
    summary: {
      totalCases,
...
export async function backfillPendingKnowledgeEmbeddings(rootDir: string, options: { projectId?: string | null; limit?: number } = {}) {
