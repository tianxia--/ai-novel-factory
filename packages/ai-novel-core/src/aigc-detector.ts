...
const LOCAL_HEURISTIC_SUMMARY_RISK_PATTERNS = [
...
function detectWithLocalHeuristic(text: string, config: AigcDetectionConfig): AigcDetectionResult {
  ...
  const summaryRiskHits = LOCAL_HEURISTIC_SUMMARY_RISK_PATTERNS
    .filter((pattern) => pattern.test(text))
    .map((pattern) => pattern.source)
  ...
  const riskScore = Math.min(
    0.98,
    0.22
      + riskHits.length * 0.21
      + summaryRiskHits.length * 0.24
      + abstractionPenalty
  ...
  const status = riskHits.length + summaryRiskHits.length >= 3 || score >= threshold
    ? "ai_likely"
