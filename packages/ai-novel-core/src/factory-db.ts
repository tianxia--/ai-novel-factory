...
import type { AutonomousNovelState, NovelProjectRecord, NovelStage } from "./cli-types"
import type { DiscussionTarget } from "./discussion"
import type { MessagePart, NovelMessage } from "./messages"
...
interface SqliteStatement {
  ...
  get(...values: DbValue[]): DbRecord | undefined
  all(...values: DbValue[]): DbRecord[]
}
...
export interface ChapterProductionFact {
...
export interface FactoryOperationalStatus {
  ok: boolean
  checkedAt: string
  projects: {
    total: number
...
const FACTORY_DB_DIR = ".ai-novel-factory"
const FACTORY_DB_FILE = "factory.sqlite"
const MIN_CHAPTER_PASS_RATIO = 0.8

function nowIso() {
  return new Date().toISOString()
}

function normalizeLlmCapability(capability: string) {
...
function stateChangeFingerprint(state: AutonomousNovelState | null) {
  if (!state) {
    return ""
  }
  const cloned = JSON.parse(JSON.stringify(state)) as AutonomousNovelState
  delete (cloned.runtime as Record<string, unknown>).lastUpdatedAt
  if (cloned.runtime.autopilot) {
    delete (cloned.runtime.autopilot as unknown as Record<string, unknown>).updatedAt
  }
...
function compactJsonString(value: unknown, maxLength = 1600) {
  const text = typeof value === "string" ? value : jsonString(value)
  if (text.length <= maxLength) return text
  return `${text.slice(0, maxLength)}...`
}

function compactDbRow(row: DbRecord): DbRecord {
  const compacted: DbRecord = { ...row }
...
function mapJobRow(row: DbRecord): DbRecord & { payload: unknown } {
  return {
    ...row,
...
function chapterNumberFromPath(value: unknown) {
  const match = String(value || "").match(/chapter-(\d+)/)
  return match ? Number(match[1]) : null
}

function normalizeQualityGate(value: unknown, updatedAt = ""): ChapterProductionFact["qualityGate"] {
  if (!value || typeof value !== "object") {
    return null
  }
  const record = value as Record<string, unknown>
  ...
  return {
    ...
    attempts: Number.isFinite(Number(record.attempts)) ? Number(record.attempts) : undefined,
    reason: typeof record.reason === "string" ? record.reason : "",
    wordCount: Number.isFinite(Number(record.wordCount)) ? Number(record.wordCount) : undefined,
    targetWords: Number.isFinite(Number(record.targetWords)) ? Number(record.targetWords) : undefined,
    updatedAt: typeof record.updatedAt === "string" ? record.updatedAt : updatedAt,
  }
}

function evaluateChapterContentQuality(
  qualityGate: ChapterProductionFact["qualityGate"],
  projectTargetWords?: number,
): ChapterProductionFact["contentQuality"] {
  if (!qualityGate) {
    return null
  }
  ...
  const wordCount = Number.isFinite(wordCountValue) ? wordCountValue : undefined
  const targetWords = Number.isFinite(targetWordsValue) ? targetWordsValue : undefined
  const minimumWords = targetWords ? Math.floor(targetWords * MIN_CHAPTER_PASS_RATIO) : undefined
...
function isWritingEventActive(payload: Record<string, unknown>, now: Date = new Date()) {
...
function isWritingEventFailed(payload: Record<string, unknown>) {
  const status = String(payload.status || "")
  const step = String(payload.step || "")
...
function normalizeTaskStatus(value: unknown): ChapterProductionFact["latestTaskStatus"] {
...
function normalizeChapterNumberList(value: unknown): number[] {
  if (!Array.isArray(value)) {
    return []
  }
  return value
    .map((entry) => {
      ...
      if (entry && typeof entry === "object" && "chapterNumber" in entry) {
        return Number((entry as Record<string, unknown>).chapterNumber)
      }
      return Number(entry)
    })
...
function applyQualityGateFact(
  fact: ChapterProductionFact,
  qualityGate: ChapterProductionFact["qualityGate"],
  projectTargetWords?: number,
) {
  if (!qualityGate) {
...
export type KnowledgeRecallRow = DbRecord & {
  source?: DbRecord
  metadata: unknown
...
export type KnowledgeChunkRow = DbRecord & {
  id: unknown
  content: unknown
  metadata: unknown
}
...
async function loadDatabaseSync() {
  ...
  const sqlite = await dynamicImport("node:sqlite") as { DatabaseSync: new (path: string) => SqliteDatabase }
  return sqlite.DatabaseSync
}

export function getFactoryDbPath(rootDir: string) {
...
export class FactoryDb {
  ...
  private migrateColumn(table: string, column: string, definition: string) {
  ...
  private migrateGraphTablePrimaryKey(table: "graph_nodes" | "graph_edges", createSql: string, columns: string) {
  ...
  upsertProject(record: NovelProjectRecord, state: AutonomousNovelState) {
    const now = nowIso()
    this.db.prepare(`
  ...
  updateProjectState(projectId: string, state: AutonomousNovelState) {
    const now = nowIso()
    const existing = this.db.prepare("SELECT state_json FROM projects WHERE id = ?").get(projectId)
    if (!existing) {
      return false
    }
    const existingState = readJson<AutonomousNovelState | null>(existing.state_json, null)
  ...
  pruneProjectsExcept(projectIds: string[]) {
  ...
  updateRun(runId: string, status: RunStatus, patch: { error?: string | null; stage?: NovelStage } = {}) {
    const now = nowIso()
    this.db.prepare(`
  ...
  recoverStaleRuns(options: { olderThan?: Date; error?: string } = {}) {
    const now = nowIso()
    const olderThan = (options.olderThan ?? new Date(Date.now() - 20 * 60 * 1000)).toISOString()
  ...
  recordAgentTurn(input: AgentTurnInput) {
    const now = nowIso()
    this.db.prepare(`
  ...
  recordArtifact(input: ArtifactRecordInput) {
    const now = nowIso()
  ...
  recordMemory(projectId: string, input: { source: string; kind: string; content: string; importance?: number; metadata?: unknown; embedding?: { model: string; vector: number[] } }) {
    const now = nowIso()
    const metadata = input.metadata ?? {}
    const metadataPath = typeof metadata === "object" && metadata && "path" in metadata
    ...
    if (metadataPath) {
      ...
      if (existing?.id) {
        const embeddingId = typeof existing.embedding_id === "string"
          ? existing.embedding_id
          : input.embedding ? makeId("emb") : null
        this.db.prepare(`
          UPDATE memory_items
  ...
  upsertEmbedding(projectId: string, input: { id?: string; ownerKind: string; ownerId: string; model: string; vector: number[] }) {
    const now = nowIso()
  ...
  markMemoryEmbeddingFailed(projectId: string, memoryId: string, error: string) {
    this.db.prepare(`
      UPDATE memory_items
  ...
  listPendingMemoryForEmbedding(projectId?: string | null, limit = 50) {
  ...
  queryMemoryByEmbedding(projectId: string, queryVector: number[], limit = 8) {
    const rows = this.db.prepare(`
    ...
    return rows
      .map((row) => {
        const vector = readJson<number[]>(row.vector_json, [])
        return {
          ...row,
      ...
      .sort((a, b) => Number(b.score) - Number(a.score) || String(b.updated_at).localeCompare(String(a.updated_at)))
      .slice(0, limit)
  }

  recallMemory(projectId: string, query = "", limit = 8, options: { embedding?: number[] } = {}) {
    ...
    return rows
      .map((row) => {
        ...
        const score = Number(row.importance || 1) * 10
          + tokens.filter((token) => content.includes(token) || kind.includes(token)).length * 20
        return {
          ...row,
          metadata: readJson(row.metadata_json, {}),
          score,
  ...
  replaceKnowledgeChunks(sourceId: string, chunks: KnowledgeChunkInput[]) {
    const now = nowIso()
    this.db.prepare(`
      UPDATE knowledge_chunks
  ...
  upsertKnowledgeEmbedding(chunkId: string, model: string, vector: number[]) {
    const now = nowIso()
  ...
  markKnowledgeEmbeddingFailed(chunkId: string, error: string) {
    this.db.prepare(`
      UPDATE knowledge_chunks
      SET embedding_status = 'failed', updated_at = ?
      WHERE id = ?
  ...
  listPendingKnowledgeChunks(options: { projectId?: string | null; scope?: KnowledgeScope; limit?: number } = {}): KnowledgeChunkRow[] {
    const safeLimit = Math.max(1, Math.min(1000, Math.floor(options.limit ?? 100)))
  ...
  queryKnowledgeByEmbedding(queryVector: number[], options: {
    ...
    return rows
      ...
      .sort((left, right) => Number(right.score) - Number(left.score) || String(right.updated_at).localeCompare(String(left.updated_at)))
      .slice(0, Math.max(1, Math.min(50, Math.floor(options.limit ?? 8))))
  }
    ...
    return rows
      .map((row) => {
        ...
        const tokenHits = tokens.filter((token) =>
          content.includes(token) || sourceType.includes(token) || chunkType.includes(token))
  ...
  recordKnowledgeCitation(input: KnowledgeCitationInput) {
    const now = nowIso()
    this.db.prepare(`
  ...
  getKnowledgeSummary(projectId: string) {
    const scalar = (sql: string, ...values: DbValue[]) => Number(this.db.prepare(sql).get(...values)?.count || 0)
  ...
  private listKnowledgeCandidateRows(options: {
    projectId?: string | null
    scopes?: KnowledgeScope[]
    sourceTypes?: string[]
    chunkTypes?: string[]
    ...
    const scopes = options.scopes?.length ? options.scopes : ["project", "global"] as KnowledgeScope[]
    const values: DbValue[] = []
    ...
    const safeLimit = Math.max(10, Math.min(1000, Math.floor((options.limit ?? 8) * 80)))
    return this.db.prepare(`
      SELECT
        knowledge_chunks.*,
  ...
  private mapKnowledgeRecallRow(row: DbRecord, score: number): KnowledgeRecallRow {
    return {
      ...compactDbRow(row),
      metadata: readJson(row.metadata_json, {}),
  ...
  upsertGraph(projectId: string, nodes: SuperGraphNode[], edges: SuperGraphEdge[]) {
  ...
  recordMessage(message: NovelMessage, parts: MessagePart[] = []) {
    const now = nowIso()
  ...
  countMessages(projectId: string, options: { conversationId?: string } = {}) {
    const row = options.conversationId
      ? this.db.prepare(`
          SELECT count(*) AS count
          FROM messages
          WHERE project_id = ? AND conversation_id = ?
        `).get(projectId, options.conversationId)
      : this.db.prepare(`
          SELECT count(*) AS count
          FROM messages
          WHERE project_id = ?
  ...
  listMessages(projectId: string, options: MessageListOptions = {}) {
    ...
    const rows = options.conversationId
      ? this.db.prepare(`
          ...
          LIMIT ? OFFSET ?
        `).all(projectId, options.conversationId, limit, offset)
      : this.db.prepare(`
          SELECT *
          FROM messages
          WHERE project_id = ?
  ...
  markStreamingMessagesFailed(projectId: string, options: {
    ...
    let changed = 0
    for (const row of rows) {
      const metadata = readJson<Record<string, unknown>>(row.metadata_json, {})
      if (
        typeof options.chapterNumber === "number"
        && Number(metadata.chapterNumber) !== options.chapterNumber
      ) {
        continue
      }
      const data = readJson<Record<string, unknown>>(row.data_json, {})
      ...
      for (const part of parts) {
        ...
        const partData = readJson<Record<string, unknown>>(part.data_json, {})
        if (partType === "json") {
          this.db.prepare(`
        ...
        if (partType === "markdown") {
          ...
          if (text) {
            const patchedText = text
              .replace(/状态：请求已提交给 LLM，等待模型开始响应。/g, `状态：${statusText}`)
              .replace(/状态：LLM 已开始响应，正在返回首段内容。/g, `状态：${statusText}`)
              .replace(/状态：LLM 正在持续返回内容。/g, `状态：${statusText}`)
              .replace(/如果模型或网络暂时没有首段返回，这条消息会保持动态等待状态。/g, statusDetail || "系统会从数据库任务恢复后重新执行。")
              .replace(/返回内容会持续合并到这一条 agent 消息中。/g, statusDetail || "系统会从数据库任务恢复后重新执行。")
          this.db.prepare(`
            UPDATE message_parts
            SET data_json = ?
            WHERE id = ?
  ...
  createJob(input: { projectId: string; runId?: string | null; kind: string; status: JobStatus; payload?: unknown }) {
    const now = nowIso()
    const id = makeId("job")
    this.db.prepare(`
  ...
  updateJobPayload(jobId: string, payload: unknown) {
    this.db.prepare(`
      UPDATE jobs
  ...
  updateJob(jobId: string, status: JobStatus, patch: { leaseOwner?: string | null; leaseExpiresAt?: string | null } = {}) {
    this.db.prepare(`
      UPDATE jobs
  ...
  listRunnableJobs(kind?: string, now: Date = new Date(), options: { includeIdle?: boolean } = {}) {
  ...
  claimJob(jobId: string, owner: string, leaseSeconds = 60, now: Date = new Date(), options: { includeIdle?: boolean } = {}) {
    const nowValue = now.toISOString()
  ...
  heartbeatJob(jobId: string, owner: string, leaseSeconds = 60, now: Date = new Date()) {
    const leaseExpiresAt = addSeconds(now, leaseSeconds).toISOString()
    this.db.prepare(`
      UPDATE jobs
  ...
  releaseJobLease(jobId: string, reason = "lease_released") {
    this.db.prepare(`
      UPDATE jobs
  ...
  completeJob(jobId: string, owner?: string | null) {
    this.finishJob(jobId, "completed", owner)
  }

  failJob(jobId: string, error: string, owner?: string | null) {
    this.finishJob(jobId, "failed", owner, { error })
  }

  pauseJob(jobId: string, owner?: string | null) {
    this.finishJob(jobId, "paused", owner)
  }

  resumeJob(jobId: string, payload?: unknown) {
    const updates = typeof payload === "undefined"
      ? "status = 'running', lease_owner = NULL, lease_expires_at = NULL, updated_at = ?"
      : "status = 'running', payload_json = ?, lease_owner = NULL, lease_expires_at = NULL, updated_at = ?"
    const values: DbValue[] = typeof payload === "undefined"
      ? [nowIso(), jobId]
      : [jsonString(payload), nowIso(), jobId]
    this.db.prepare(`
      UPDATE jobs
      SET ${updates}
      WHERE id = ?
  ...
  jobFailureLooksRecoverable(jobId: string) {
    ...
    return !/autopilot stopped by user|abort/i.test(error)
      && /network|fetch|failed|econn|enotfound|etimedout|socket|undici|timeout|timed\s*out|timed-out|429|rate limit|too many requests|quota|503|502|504|temporar|unavailable|overloaded|bad gateway|gateway timeout/i.test(error)
  }

  cancelJob(jobId: string, owner?: string | null) {
    this.finishJob(jobId, "cancelled", owner)
  }

  listProjectJobs(projectId: string, kind?: string) {
    const rows = kind
      ? this.db.prepare(`
          SELECT * FROM jobs
  ...
  cancelProjectJobs(projectId: string, kind?: string) {
    const jobs = this.listProjectJobs(projectId, kind)
  ...
  private finishJob(jobId: string, status: JobStatus, owner?: string | null, metadata: Record<string, unknown> = {}) {
    const ownerClause = owner ? "AND (lease_owner = ? OR lease_owner IS NULL)" : ""
    const values: DbValue[] = owner
      ? [status, nowIso(), jobId, owner]
      : [status, nowIso(), jobId]
    this.db.prepare(`
      UPDATE jobs
  ...
  getChapterFacts(projectId: string, now: Date = new Date()): ChapterProductionFact[] {
    const facts = new Map<number, ChapterProductionFact>()
    ...
    const targetWordsForQuality = Number.isFinite(projectTargetWords) ? projectTargetWords : undefined
    const projectRoot = typeof projectRow?.project_root === "string" ? projectRow.project_root : ""
    const protagonistProfile = projectRoot
      ? readTextFileSync(path.join(projectRoot, ".ai-novel", "memory", "characters", "core", "protagonist.md"))
      : ""
    const ensureFact = (chapterNumber: number): ChapterProductionFact => {
      const existing = facts.get(chapterNumber)
      if (existing) return existing
      const created: ChapterProductionFact = { chapterNumber, status: "pending" }
      facts.set(chapterNumber, created)
      return created
    }
    const resetAtByChapter = new Map<number, string>()
    ...
    const isResetHistory = (chapterNumber: number, value: unknown) => {
    ...
    for (const fact of [...facts.values()].sort((left, right) => left.chapterNumber - right.chapterNumber)) {
      ...
      const failedEventIsNewer = latestEventFailed
        && latestEventAt > 0
        && latestEventAt >= Math.max(latestTaskStatusAt, qualityGateUpdatedAt)
      if (
        isWritingEventActive({
          status: fact.latestEventStatus || "",
          step: fact.latestStep || "",
          createdAt: fact.latestEventAt || "",
        }, now)
        && fact.qualityGate?.status !== "blocked"
      ) {
        fact.status = "in_progress"
      } else if (
        fact.qualityGate?.status === "passed"
        && fact.finalPath
        && fact.contentQuality?.status === "eligible"
      ) {
  ...
  getSnapshot(projectId: string): ProjectSnapshot {
    ...
    const projectRow = this.db.prepare("SELECT state_json, project_root FROM projects WHERE id = ?").get(projectId) as { state_json: string; project_root: string } | undefined
    ...
    const quarantinedFinalFacts = chapterFacts.filter((fact) =>
      Boolean(fact.finalPath) && fact.contentQuality?.status === "quarantined")
    const untrustedPassedGateFacts = quarantinedFinalFacts.filter((fact) => fact.qualityGate?.status === "passed")
    const latestTrustedFinal = completedChapterFacts
      .slice()
    ...
    const recentArtifacts = this.db.prepare(`
      ...
      LIMIT 80
    `).all(projectId)
    const artifactsByPath = new Map<string, DbRecord>()
    ...
    const stateObj = readJson<AutonomousNovelState | null>(projectRow?.state_json, null)
    ...
    if (projectRoot && stateObj) {
      ...
      const activeTask = tasks.find((t: any) => t.status === "in_progress" || t.status === "blocked")
      ...
      for (const [pathKey, row] of artifactsByPath.entries()) {
        ...
        if (isOutline || isBlueprint || isDraft || isLatestDiscussion) {
          try {
            const absPath = fsSync.existsSync(pathKey)
              ? pathKey
              : path.resolve(projectRoot, pathKey)
            if (fsSync.existsSync(absPath)) {
    ...
    const latestEventRows = this.db.prepare(`
      ...
      LIMIT 80
    `).all(projectId).map(compactDbRow)
    const milestoneEventTypes = new Set(["PROJECT_CREATED", "CONSENSUS_UPDATED", "CHAPTER_PIPELINE_COMPLETED"])
  ...
  getOperationalStatus(now: Date = new Date()): FactoryOperationalStatus {
    const nowValue = now.toISOString()
    const scalar = (sql: string, ...values: DbValue[]) => Number(this.db.prepare(sql).get(...values)?.count || 0)
  ...
  listSystemSettings(): Array<{ key: string; value: string }> {
    return this.db.prepare(`
...
export function makeRunId(kind: string) {
  return makeId(`run_${kind}`)
}

export function makeAgentTurnId(runId: string, agentId: string, index: number) {
  return `${runId}:${agentId}:${index}`
}

export function targetToArtifactKind(target: DiscussionTarget): ArtifactRecordInput["kind"] {
