...
export async function generateAgentReply(options: AgentReplyOptions) {
  ...
  if (options.onDelta) {
    return streamOpenAiCompatibleResponse(response, options.onDelta)
  }
...
export async function testProviderConnectivity(overrides: ProviderOverrideOptions = {}): Promise<ProviderTestResult> {
  const config = loadLlmConfigFromEnv()
  const envStatus = getProjectEnvStatus()
